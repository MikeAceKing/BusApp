...
import { AddressText, BottomNav, BusAvatar, ErrorBanner, FriendlyBus, HonestMapState, InitialAvatar, PageHeader, StateCard, createT } from './Shared';
...
export function ParentBusHome({ grantId, locale, onExit, onGrantLost }: { grantId: string; locale: Locale; onExit: () => void; onGrantLost: () => void }) {
  ...
  return <main className="app-shell parent-home">
    ...
    <div className="page-content">
      ...
      {tab === 'profile' && <section className="profile-page"><header className="section-heading"><div><small>BusApp</small><h1>{t('profile')}</h1></div></header><section className="profile-identity"><InitialAvatar name={data.parent.displayName} avatar="initials-orange" /><div><h2>{data.parent.displayName}</h2><p>{data.passengers.map((item) => item.display_name).join(' · ')}</p></div></section><h2 className="subsection-title">{t('privacy')}</h2><aside className="privacy-note"><ShieldCheck aria-hidden="true" /><span><strong>{t('privacyTitle')}</strong><small>{t('privacyBody')}</small></span></aside><button className="danger-link logout-button button-with-icon" onClick={async () => { await supabase.auth.signOut(); onGrantLost(); }}><LogOut aria-hidden="true" />{t('logout')}</button></section>}
    </div>
    <BottomNav items={[
