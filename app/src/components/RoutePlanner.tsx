...
import { BusyButton, ErrorBanner, HonestMapState, createT } from './Shared';
...
export function RoutePlanner({ home, locale, onChanged, onStart }: { home: SpaceHome; locale: Locale; onChanged: () => Promise<void>; onStart: (plan: RoutePlan) => Promise<void> }) {
  ...
  return <section className="route-page">
    <header className="section-heading"><div><small>BusApp</small><h1>{plan ? (estimate ? t('routeEstimate') : t('realRoute')) : t('routeMissing')}</h1></div>{plan && <RouteIcon className="section-heading__icon" aria-hidden="true" />}</header>

    {!plan && <HonestMapState title={t('routeMissing')} body={t('routeMissingHelp')} />}
    {plan && estimate && <HonestMapState title={t('routeEstimate')} body={t('estimated')} />}
    ...
    {plan && <div className={`route-summary ${estimate ? 'route-summary--estimate' : ''}`}>
      <div><Signpost aria-hidden="true" /><strong>{estimate ? '± ' : ''}{(plan.distance_meters / 1000).toFixed(1)}</strong><small>{t('kilometers')}</small></div>
      <div><Clock3 aria-hidden="true" /><strong>{estimate ? '± ' : ''}{Math.max(1, Math.round(plan.duration_seconds / 60))}</strong><small>{t('minutes')}</small></div>
      <div><MapPinned aria-hidden="true" /><strong>{plan.stops.length}</strong><small>{t('stops')}</small></div>
    </div>}
    {plan && estimate && <p className="estimate-note">{t('estimated')}</p>}
    ...
    <div className="route-stop-list">{orderedIds.map((id, index) => {
      ...
      return <article key={id} draggable={manual} onDragStart={() => setDrag(id)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (!drag || drag === id) return; const next = order.filter((item) => item !== drag); next.splice(next.indexOf(id), 0, drag); setOrder(next); setDrag(null); }}>
        <b>{index + 1}</b><span><strong>{stop.label || stop.display_address.split(',')[0]}</strong><small>{stop.display_address} · {stop.expected_passenger_count} {t('children')}</small></span>
    ...
    <BusyButton busy={busy === 'route'} className="primary-button jumbo button-with-icon" disabled={!home.stops.length} onClick={calculate}><RouteIcon aria-hidden="true" />{plan ? t('recalculate') : t('calculateRoute')}</BusyButton>
    {plan && <BusyButton busy={busy === 'start'} className="start-button jumbo button-with-icon" onClick={async () => { setBusy('start'); setError(''); try { await onStart(plan); } catch (reason) { setError(reason instanceof Error ? reason.message : t('error')); } finally { setBusy(''); } }}><Play aria-hidden="true" />{t('startTrip')}</BusyButton>}
  </section>;
}
