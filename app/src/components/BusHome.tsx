...
import type { BusSpaceRole, Locale, RoutePlan, SpaceHome, SpaceSummary } from '../types';
import { AddressText, BottomNav, BusyButton, CountLabel, ErrorBanner, FriendlyBus, FriendlyRouteIllustration, StateCard, createT } from './Shared';
import { BusProfileCard, UserProfileEditor } from './AvatarProfiles';
...
export function BusHome({ space, locale, onLocale, onExit }: { space: SpaceSummary; locale: Locale; onLocale:(locale:Locale)=>void; onExit: () => void }) {
  ...
  useEffect(()=>{let channel:ReturnType<typeof supabase.channel>|null=null;let disposed=false;void supabase.auth.getUser().then(({data})=>{if(disposed||!data.user)return;channel=supabase.channel(`bus-app-avatars-${data.user.id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'bus_app_avatar_updates',filter:`user_id=eq.${data.user.id}`},()=>{void load();}).subscribe();});return()=>{disposed=true;if(channel)void supabase.removeChannel(channel);};},[load]);
  ...
  if (home.activeTrip) return <main className="app-shell runtime-shell"><TripRuntime spaceId={space.id} locale={locale} onFinished={load} /></main>;
  if (adding) return <main className="app-shell"><div className="page-content"><header className="section-heading"><div><small>{home.bus?.name}</small><h1>{t('addAddress')}</h1></div></header><StopEditor spaceId={space.id} locale={locale} onCancel={() => setAdding(false)} onDone={async () => { setAdding(false); await load(); }} /></div></main>;
  ...
  return <main className="app-shell bus-home">
    <div className="page-content">
      ...
      {tab === 'profile' && <Profile home={home} locale={locale} onLocale={onLocale} onChanged={load} onExit={onExit} canManageBus={home.permissions?.manageBusProfile ?? space.roles.includes('OWNER')} role={home.role || space.roles[0]} />}
    </div>
    <BottomNav items={[
...
function Profile({ home, locale, onLocale, onChanged, onExit, canManageBus, role }: { home: SpaceHome; locale: Locale; onLocale:(locale:Locale)=>void;onChanged:()=>Promise<void>;onExit:()=>void;canManageBus:boolean;role?:BusSpaceRole }) {
  const t = createT(locale);
  ...
  const memberSummary = `${home.members.length} ${home.members.length === 1 ? t('memberOne') : t('memberMany')} · ${home.space.default_language === 'fr' ? t('languageFrench') : t('languageDutch')}`;
  return <section className="profile-page">
    <header className="section-heading"><div><small>BusApp</small><h1>{t('profile')}</h1></div></header>
    <UserProfileEditor profile={home.profile} locale={locale} onLocale={onLocale} onChanged={onChanged} role={role}/>
    <h2 className="subsection-title">{t('myBusSection')}</h2>
    {home.bus && <BusProfileCard busId={home.bus.id} spaceId={home.space.id} name={home.bus.name} avatar={home.bus.avatar} subtitle={memberSummary} locale={locale} onChanged={onChanged} editable={canManageBus}/>}
