...
app.get('/parent/home',async(c)=>{try{
  ...
  let location:Row|null=null;let statuses:Row[]=[];let tripStops:Row[]=[];let progressStops:Row[]=[];if(trip.data){const [locationResult,statusResult,tripStopResult,progressResult]=await Promise.all([db.from('bus_app_trip_live_locations').select('latitude,longitude,captured_at').eq('trip_id',trip.data.id).maybeSingle(),db.from('bus_app_trip_passenger_statuses').select('passenger_id,status,version,trip_stop_id').eq('trip_id',trip.data.id).in('passenger_id',passengerIds),db.from('bus_app_trip_stops').select('id,source_stop_id,sequence,status,estimated_arrival_offset_seconds').eq('trip_id',trip.data.id).in('source_stop_id',stopIds),db.from('bus_app_trip_stops').select('sequence,status,completed_at,estimated_arrival_offset_seconds').eq('trip_id',trip.data.id).order('sequence')]);if(locationResult.error)throw locationResult.error;if(statusResult.error)throw statusResult.error;if(tripStopResult.error)throw tripStopResult.error;if(progressResult.error)throw progressResult.error;location=locationResult.data?{latitude:Math.round(numberValue(locationResult.data.latitude)*1000)/1000,longitude:Math.round(numberValue(locationResult.data.longitude)*1000)/1000,capturedAt:locationResult.data.captured_at}:null;statuses=rows(statusResult.data);tripStops=rows(tripStopResult.data);progressStops=rows(progressResult.data);}
  // The scheduled offsets are fixed at departure, so counting down from started_at drifts
  // the moment the bus runs late: a parent ten stops down the line would watch the estimate
  // reach zero while the bus is nowhere near them. Anchor instead on the last stop the bus
  // actually completed, and add only the remaining scheduled travel from there. Delay is
  // absorbed because the anchor moves with reality.
  const servedStops=progressStops.filter((item)=>['COMPLETED','SKIPPED'].includes(text(item.status))&&text(item.completed_at));
  const anchorStop=servedStops.length?servedStops[servedStops.length-1]:null;
  const anchorAt=anchorStop?Date.parse(text(anchorStop.completed_at)):Date.parse(text(trip.data?.started_at));
  const anchorOffset=anchorStop?numberValue(anchorStop.estimated_arrival_offset_seconds):0;
  await db.from('bus_app_parent_grants').update({last_seen_at:new Date().toISOString()}).eq('id',grant.data.id);
  const decoratedPassengers=await withAvatars(rows(passengers.data));return c.json({grantId:grant.data.id,parent:{displayName:text(profile.display_name)||access.data.parent_display_name,profile:{...profile,avatar:await avatarReference(profile)}},space:space.data,bus:await withAvatar(bus.data as Row|null),trip:trip.data?{id:trip.data.id,status:trip.data.status,currentStopSequence:trip.data.current_stop_sequence,startedAt:trip.data.started_at,location}:null,passengers:decoratedPassengers.map((passenger)=>{const stop=rows(stops.data).find((item)=>item.id===passenger.stop_id)||null;const status=statuses.find((item)=>item.passenger_id===passenger.id);const tripStop=tripStops.find((item)=>item.source_stop_id===passenger.stop_id);const mySequence=numberValue(tripStop?.sequence);const remainingSeconds=tripStop?Math.max(0,numberValue(tripStop.estimated_arrival_offset_seconds)-anchorOffset):0;const rawEta=trip.data?.started_at&&tripStop&&Number.isFinite(anchorAt)?Math.ceil((anchorAt+remainingSeconds*1000-Date.now())/60_000):null;const stopsAway=tripStop?progressStops.filter((item)=>numberValue(item.sequence)<mySequence&&!['COMPLETED','SKIPPED'].includes(text(item.status))).length:null;const eta=rawEta===null?null:rawEta>0?rawEta:(stopsAway||0)>0?null:0;return {...passenger,stop,status:status?.status||'EXPECTED',statusVersion:numberValue(status?.version,1),etaMinutes:eta,stopsAway};})});
}catch(error){return handleError(c,error);}});
...
// A parent's map context is built from their own grant only. Route geometry is trimmed to
// the span between the bus and that parent's own stop, so a full-route polyline through
// every family's street never reaches a parent device.
function nearestVertexIndex(coordinates:Array<[number,number]>,longitude:number,latitude:number):number{
  let best=0;let bestDistance=Number.POSITIVE_INFINITY;
  coordinates.forEach((point,index)=>{const dx=point[0]-longitude;const dy=point[1]-latitude;const distance=dx*dx+dy*dy;if(distance<bestDistance){bestDistance=distance;best=index;}});
  return best;
}

function parentRouteGeometry(plan:Row|null,bus:{longitude:number;latitude:number}|null,stop:{longitude:number;latitude:number}|null):Row|null{
  if(!plan||!bus||!stop)return null;
  // Only geometry the provider actually returned as road geometry may ever be sent.
  const metadata=(plan.provider_metadata||{}) as Row;
  if(text(metadata.geometrySource)!=='road')return null;
  const geometry=(plan.route_geometry||{}) as Row;
  if(text(geometry.type)!=='LineString'||!Array.isArray(geometry.coordinates))return null;
  const coordinates=(geometry.coordinates as unknown[]).filter((entry):entry is [number,number]=>Array.isArray(entry)&&entry.length>=2&&Number.isFinite(Number(entry[0]))&&Number.isFinite(Number(entry[1]))).map((entry)=>[Number(entry[0]),Number(entry[1])] as [number,number]);
  if(coordinates.length<2)return null;
  const from=nearestVertexIndex(coordinates,bus.longitude,bus.latitude);
  const to=nearestVertexIndex(coordinates,stop.longitude,stop.latitude);
  const slice=from<=to?coordinates.slice(from,to+1):coordinates.slice(to,from+1).reverse();
  return slice.length>=2?{type:'LineString',coordinates:slice}:null;
}
...
app.get('/parent/bus-profile',async(c)=>{try{
  ...
  const trip=await db.from('bus_app_trips').select('id,status,created_by,route_plan_id').eq('bus_id',busId).in('status',['BOARDING','IN_TRANSIT','ARRIVED']).order('created_at',{ascending:false}).limit(1).maybeSingle();if(trip.error)throw trip.error;
  const staff=await parentVisibleStaff(spaceId,text(trip.data?.created_by)||null);
  const ownStop=rows(stops.data)[0]||null;
  const ownStopPoint=ownStop&&Number.isFinite(numberValue(ownStop.latitude))?{longitude:numberValue(ownStop.longitude),latitude:numberValue(ownStop.latitude)}:null;
  let busPoint:{longitude:number;latitude:number}|null=null;let plan:Row|null=null;
  if(trip.data){
    const [live,planResult]=await Promise.all([
      db.from('bus_app_trip_live_locations').select('latitude,longitude,captured_at').eq('trip_id',text(trip.data.id)).maybeSingle(),
      db.from('bus_app_route_plans').select('route_geometry,provider_metadata').eq('id',text(trip.data.route_plan_id)).maybeSingle(),
    ]);
    if(live.error)throw live.error;if(planResult.error)throw planResult.error;
    // Same rounding as every other parent-facing coordinate: the bus, never a person.
    if(live.data)busPoint={longitude:Math.round(numberValue(live.data.longitude)*1000)/1000,latitude:Math.round(numberValue(live.data.latitude)*1000)/1000};
    plan=(planResult.data||null) as Row|null;
  }
  const map={bus:busPoint,ownStop:ownStopPoint?{...ownStopPoint,displayLabel:text(ownStop?.display_address)}:null,etaSeconds:null,routeGeometry:parentRouteGeometry(plan,busPoint,ownStopPoint)};
  return c.json({bus:{displayName:text(bus.data.name),avatar:await avatarReference(bus.data as Row),currentTripStatus:trip.data?text(trip.data.status):null},driver:staff.driver,attendant:staff.attendant,ownStop:ownStop?{displayAddress:text(ownStop.display_address)}:null,map});
}catch(error){return handleError(c,error);}});
...
app.post('/trips/:tripId/stops/:tripStopId',async(c)=>{try{const {access,trip}=await tripAccess(c.req.raw,c.req.param('tripId'),['OWNER','DRIVER','ATTENDANT']);const parsed=stopActionSchema.safeParse(await c.req.json().catch(()=>null));if(!parsed.success)throw new HttpError(400,'INPUT_INVALID','Controleer de halteactie.');const map={APPROACH:'APPROACHING',ARRIVE:'AT_STOP',COMPLETE:'COMPLETED',SKIP:'SKIPPED'} as const;const timestamps:Row={};if(parsed.data.action==='APPROACH')timestamps.approached_at=new Date().toISOString();if(parsed.data.action==='ARRIVE')timestamps.arrived_at=new Date().toISOString();if(parsed.data.action==='COMPLETE')timestamps.completed_at=new Date().toISOString();const updated=await db.from('bus_app_trip_stops').update({status:map[parsed.data.action],...timestamps}).eq('id',c.req.param('tripStopId')).eq('trip_id',trip.id).select('*').maybeSingle();if(updated.error)throw updated.error;if(!updated.data)throw new HttpError(404,'TRIP_STOP_NOT_FOUND','Halte niet gevonden.');if(['COMPLETE','SKIP'].includes(parsed.data.action))await db.from('bus_app_trips').update({current_stop_sequence:updated.data.sequence}).eq('id',trip.id);
  if(['APPROACH','COMPLETE','SKIP'].includes(parsed.data.action)){
    try{
      const affected=await db.from('bus_app_trip_passenger_statuses').select('passenger_id').eq('trip_id',trip.id);
      if(affected.error)throw affected.error;
      const recipients=await parentRecipientsForPassengers(rows(affected.data).map((item)=>text(item.passenger_id)).filter(Boolean));
      if(recipients.targets.length){
        const pulsed=await db.from('bus_app_parent_trip_updates').insert(recipients.targets.map((target)=>({user_id:target.userId,parent_access_id:target.parentAccessId,trip_id:trip.id,event_type:'STOP_PROGRESS'})));
        if(pulsed.error)throw pulsed.error;
      }
    }catch(reason){console.error('[bus-app] parent stop pulse failed after a successful stop update',reason);}
  }
  await audit(access,`STOP_${parsed.data.action}`,{tripId:text(trip.id),metadata:{tripStopId:updated.data.id}});return c.json({stop:updated.data});}catch(error){return handleError(c,error);}});
